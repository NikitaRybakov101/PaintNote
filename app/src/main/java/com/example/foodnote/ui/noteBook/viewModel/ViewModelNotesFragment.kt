        ...
        scope.launch {
            ...
            withContext(Dispatchers.Main) {
                ...
                dataStandardNotes.forEach { not ->
                    val note = NoteStandard(not.widthCard,not.heightCard,not.colorCard,not.note,not.font,not.fontSize,not.cardPositionX,not.cardPositionY,not.idCard,not.elevation.toFloat())
    ...
    override fun saveDataNotesStandard(note : NoteStandard): Unit = with(note){
        scope.launch {
            notesDao.insertNoteStandard(
                ...
                widthCard = widthCard, heightCard = heightCard, colorCard = colorCard, note = string, font = font, fontSize = fontSize,
                cardPositionX = posX, cardPositionY = posY, idCard = idNote, elevation = elevationNote.toInt() )
            )
        }
    }
